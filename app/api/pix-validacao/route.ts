...
export async function POST(request: NextRequest) {
  try {
    ...
    const amountCents = Math.round((amount || 18.25) * 100)
    ...
    const response = await fetch("https://app.bosspay.cash/api/v1/pix/create", {
      ...
      body: JSON.stringify({
        amount_cents: amountCents, // R$ 18,25
        customer_name: customerName,
        customer_document: cleanDocument,
        customer_phone: customerPhone.replace(/\D/g, ""),
        customer_email: customerEmail || undefined,
        description: "Validação Bancária Obrigatória",
    ...
    console.error("[v0] Error creating PIX Validacao:", error)
    return NextResponse.json(
